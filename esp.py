...
class LoRa:
    def __init__(self, RST_Pin, CS_Pin, SPI_CH, SCK_Pin, MOSI_Pin, MISO_Pin, DIO0_Pin, plus20dBm=False):
        ...
        self.write('RegModemConfig1', Bw['500KHz'] << 4 | CodingRate[5] << 1 | ImplicitHeaderModeOn['Explicit'])
        SpreadingFactor = {7: 0x7, 9: 0x9, 12: 0xC}
        TxContinuousMode = {'normal': 0b0, 'continuous': 0b1}
        RxPayloadCrcOn = {'disable': 0b0, 'enable': 0b1}
        self.write('RegModemConfig2', SpreadingFactor[12] << 4 | TxContinuousMode['normal'] << 3 | RxPayloadCrcOn[
            'enable'] << 2 | 0x00)
        ...
        # It enables an interrupt when Tx is done.
        '''
        self.DioMapping = {
            'Dio0': {
                'RxDone': 0b00 << 6,
...
def send_to_pi(data):
    ...
    try:
        ...
        print("Error sending data to Pi:", e)
